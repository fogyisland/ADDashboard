...
// ===== R97: search box removed =====
// Operator directive 2026-09-25: "去掉搜索菜单".
//
// R92 (2026-09-24) introduced:
//   1. <input class="sidebar-search"> rendering only when sidebar was visible
//   2. Typing filtering items case-insensitively by label
//   3. Groups with zero matches removed (compact result)
//   4. Empty query restoring all 7 groups / 22 links
//   5. Clear (✕) button visible when query was non-empty
//   6. Source CSS giving <nav> max-height + overflow-y so long menus
//      got a scrollbar
// R97 retracts all of (1)–(5). (6) survives because the scroll budget
// is still needed for the un-filtered 7-group / 22-link menu — covered
// by the surviving R53/R54 source-CSS test at admin-layout.test.js:361-366
// (`expect(body).toMatch(/max-height/); expect(body).toMatch(/overflow-y:\s*auto/);`).
//
// The 19 R53/R54/R75/R94 tests at the top of this file continue to pass
// verbatim because removing search is the identity case: <details> renders
// the full groups array, which the existing tests already lock.