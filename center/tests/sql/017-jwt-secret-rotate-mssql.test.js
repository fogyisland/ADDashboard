// 017-jwt-secret-rotate-mssql.test.js — real-MSSQL integration test for
// the I9 dual-key JWT secret rotation SQL surface (Tasks 1-5).
//
// Pattern follows 016-agent-token-rotate-mssql.test.js: driver-level,
...
// against a live MSSQL driver so the actual product SQL —
// `db.sql.config.upsert` (MSSQL MERGE), `db.sql.config.getJwtSecretBundle`,
...
test('jwt_secret rotate + commit round-trip on system_config (mssql)', { skip: !MSSQL }, async () => {
  ...
  try {
    ...
    const auditsResult = await db.query(
      ...
      ['rotate_jwt_secret', 'system_config']
    );
    ...
    const commitAuditsResult = await db.query(
      ...
      ['commit_jwt_secret', 'system_config']
    );
    ...
    try {
      ...
      console.error(`[017-mssql] snapshot restore failed: ${e.message}`);
    }
    try {
      await db.execute(
        `DELETE FROM audit_logs WHERE action IN (@p1, @p2, @p3, @p4) AND target = @p5`,
        ['rotate_jwt_secret', 'commit_jwt_secret', 'seed_jwt_secret', 'auto_expire_jwt_secret', 'system_config']
      );
    } catch {
