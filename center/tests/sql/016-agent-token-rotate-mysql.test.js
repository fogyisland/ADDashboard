// 016-agent-token-rotate-mysql.test.js — real-MySQL integration test for
// the I3 dual-key agent-token rotation SQL surface (Tasks 1-5).
//
// Pattern follows 015-user-token-version-mysql.test.js: driver-level,
...
// `commitAgentToken`) against a live MySQL driver so the actual product
// SQL — `db.sql.config.upsert`, `db.sql.config.getAgentTokenBundle`,
...
test('agent_token rotate + commit round-trip on system_config (mysql)', { skip: !MYSQL }, async () => {
  ...
  try {
    ...
    const { rows: audits } = await db.query(
      `SELECT action, target, payload FROM audit_logs WHERE action = 'rotate_agent_token' AND target = 'system_config' ORDER BY id DESC LIMIT 1`,
      []
    );
    ...
    assert.equal(audits[0].payload.previousLength, '__test_i3_old__'.length,
      'previousLength matches pre-rotate current (15 chars)');
    assert.equal(audits[0].payload.newLength, 96, 'newLength is 48-byte hex (96 chars)');
    ...
    const { rows: commitAudits } = await db.query(
      `SELECT action, target FROM audit_logs WHERE action = 'commit_agent_token' AND target = 'system_config' ORDER BY id DESC LIMIT 1`,
      []
    );
    ...
    try {
      ...
      console.error(`[016-mysql] snapshot restore failed: ${e.message}`);
    }
    try {
      await db.execute(
        `DELETE FROM audit_logs WHERE action IN ('rotate_agent_token','commit_agent_token','seed_agent_token') AND target = 'system_config'`,
        []
      );
    } catch {
