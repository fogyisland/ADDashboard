...
// Regression test (whole-branch fix round 1, C1):
// The previous version of email.send() destructured smtp.host/port/user/password
// (camelCase) but the loop passes the snake_case system_config map. Every send
// attempt with SMTP configured was passing host: undefined to nodemailer.
// Existing tests masked this because they injected sendImpl and never
// exercised the real send() through tick().
//
// This test drives tick() through the REAL send() (no sendImpl) with a stubbed
// createTransport forwarded via _deps. It asserts createTransport was called
// with the correct snake_case keys, proving the destructure was fixed.
test('emailOutboxLoop: tick drives real send() with snake_case SMTP config (C1 regression)', async () => {
  const now = new Date('2026-01-01T12:00:00Z');
  const pendingRow = {
    id: 10,
    alert_event_id: 100,
  ...
  // Capture the createTransport call args to prove snake_case was read.
  let captured = null;
  const createTransport = (opts) => {
    captured = opts;
    return { sendMail: async () => ({ messageId: 'msg-1' }) };
  };

  // Build the loop directly (NOT via makeLoop which defaults sendImpl).
  // We pass getSystemConfig returning the snake_case map (the real shape
  // returned by config.getConfigMap in production) and _deps containing
  // our stub createTransport.
  const db = buildMockDb([
    { match: /FROM alert_email_outbox/i, rows: [pendingRow] }
  ]).withRecording();

  const loop = createEmailDeliveryLoop({
    db,
    getIntervalSeconds: async () => 60,
    ...
    _deps: { createTransport }
  });

  await loop.tick();

  // createTransport was called with the snake_case SMTP config — proves
  // send() now reads smtp.smtp_host/port/user/password correctly.
  assert.ok(captured, 'createTransport should have been invoked');
  assert.equal(captured.host, 'smtp.example.com');
  assert.equal(captured.port, 587);
  assert.equal(captured.secure, false);
  assert.deepEqual(captured.auth, { user: 'user', pass: 'pass' });

  // The outbox UPDATE for sent_at should have run after a successful send.
  const markSent = findExecute(db.records, (sql, params) =>
    /UPDATE alert_email_outbox SET sent_at/i.test(sql) && params.includes(10)
  );
  assert.ok(markSent, 'markSent UPDATE should have been issued on success');
});
