<!--
  2026-08-31 R75 — UserDeleteConfirmModal.vue.
  Confirmation requires operator to type the sAMAccountName.
  Submits `user_delete` command.
  data-test: user-delete-confirm-modal / input / submit / result.
-->
<template>
  <div class="modal-bg" @click.self="cancel">
    <div class="modal" data-test="user-delete-confirm-modal">
      <header><h3>删除用户 — {{ sam }}</h3></header>
      <section class="form-body">
        <p class="warn-banner">
          ⚠ 警告：此操作不可撤销。AD 中该用户及其全部属性将被永久删除。
        </p>
        <p class="hint">请输入 sAMAccountName <code>{{ sam }}</code> 以确认删除：</p>
        <label class="field">
          <span class="label">sAMAccountName</span>
          <input
            data-test="user-delete-confirm-input"
        ...
        <p v-if="resultMessage" :class="['result', resultOk ? 'ok' : 'err']" data-test="user-delete-confirm-result">{{ resultMessage }}</p>
      </section>
      <footer>
        <button type="button" data-test="user-delete-confirm-cancel" @click="cancel" :disabled="submitting">取消</button>
        <button
          type="button"
          data-test="user-delete-confirm-submit"
          class="danger"
          @click="submit"
          :disabled="submitting || confirmInput !== sam || !props.targetDc"
...
async function submit() {
  if (confirmInput.value !== props.sam) {
    error.value = 'sAMAccountName 不匹配';
    return;
  }
  error.value = '';
