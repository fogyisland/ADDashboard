...
仅做前端调试 / 不想动默认安装路径（脚本相对的 `<publish-root>\Center`）的轻量场景见本文末尾的 [附录：本地纯前端开发模式](#附录本地纯前端开发模式)。
...
2. 确保 `frontend/dist/` 已构建 → 镜像到 `system\center\dist\`
...
│           ├── Service.psm1
│           └── Ensure-Nssm.ps1
├── designer\                             ← WPF 包设计器（自包含 exe，直接双击运行）
└── installer\                            ← MSI 安装包源码 + build 脚本
```
