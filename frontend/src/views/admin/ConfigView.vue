...
const config = ref({});
const descriptions = { polling_interval_minutes: '采集周期 (分钟)', latency_threshold_minutes: '复制延迟告警阈值 (分钟)', heartbeat_interval_seconds: 'Agent 心跳间隔 (秒), 默认 5, 越短越快感知掉线', history_enabled: '是否写入历史快照 (0/1)', ad_agent_token: 'Agent 共享 Token', center_public_host: '对外域名/IP (给 Agent / 用户访问用, 例如 ad-dashboard.contoso.com 或 10.1.2.3)', center_public_port: '对外端口 (例如 443=HTTPS, 80=HTTP)' };
