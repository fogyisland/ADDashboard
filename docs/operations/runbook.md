...
`start.ps1` 内部行为：
- service 未注册 → 若 `center/web/` 源码存在，按 `package.json` 中可用的 build 脚本（`build` / `build:web` / `build:frontend`）重新生成 dist，然后 `install-center.ps1 -InPlace` 注册 NSSM service 并启动。
- service 已注册 → 同上重建 dist，然后优先 `POST http://localhost:8080/api/system/update`（localhost-only，no-auth；自动跑 pending migration + `process.exit(0)` + NSSM 用新代码拉起）。
