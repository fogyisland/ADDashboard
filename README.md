...
仅做前端调试 / 不动默认安装路径的轻量场景见本文末尾的 [附录：本地纯前端开发模式](#附录本地纯前端开发模式)。
...
绿色版默认就在 `publish\center\` 跑服务（日志写到 `$DashboardRoot\Logs\`）。多机批量安装（含远程 WinRM 部署 Agent）见仓库根目录的 [`docs/operations/deployment.md`](docs/operations/deployment.md) 和 [`scripts/README.md`](scripts/README.md)。
